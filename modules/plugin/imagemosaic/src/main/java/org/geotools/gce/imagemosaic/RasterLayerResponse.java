 ...
 */
@SuppressWarnings("rawtypes") class RasterLayerResponse {
    ...
    private class GranuleCollector {
        ...
        private GranuleCollector(Filter granuleFilter, boolean dryRun) {
            ...
            doInputTransparency =
                    inputTransparentColor != null && !footprintBehavior.handleFootprints();
        }
            ...
            for (Future<GranuleLoadingResult> future : granulesFutures) {
                ...
                try {
                    ...
                    if (result == null) {
                        if (LOGGER.isLoggable(Level.FINE)) {
                            LOGGER.log(Level.FINE,
                                    "Unable to load the raster for granule with request " + request
                                            .toString());
                        }
                        continue;
                    }
                        ...
                        if (LOGGER.isLoggable(Level.FINE)) {
                            LOGGER.log(Level.FINE,
                                    "Unable to load the raster for granuleDescriptor "
                                            + result.granuleUrl + " with request " + request
                                            .toString());
                        }
                        continue;
                    }
                    ...
                    if (sourceThreshold == null) {
                        ...
                        //
                        sourceThreshold = new double[][] { { CoverageUtilities.getMosaicThreshold(
        ...
        private MosaicElement preProcessGranuleRaster(RenderedImage granule,
            ...
            if (hasAlpha || doInputTransparency) {
                ...
                if (granule.getSampleModel() instanceof MultiPixelPackedSampleModel || granule
            ...
            Geometry mask = JTS.toGeometry(
                    new Envelope(bounds.getMinX(), bounds.getMaxX(), bounds.getMinY(),
                            bounds.getMaxY()));
            ...
            if (footprintBehavior.handleFootprints()) {
                ...
                // ARTIFACTS FILTERING
                if (defaultArtifactsFilterThreshold != Integer.MIN_VALUE && result
                        .isDoFiltering()) {
    ...
    private class Mosaicker {
        ...
        private MosaicElement createMosaic() throws IOException {
                        ...
                        if (imageBounds.equals(bounds)) {
                            ...
                            if (!imageBounds.contains(rasterBounds)) {
                                mosaic = MergeBehavior.FLAT
                                        .process(new RenderedImage[] { mosaic }, backgroundValues,
                                                sourceThreshold, (hasAlpha || doInputTransparency) ?
                                                        new PlanarImage[] { in.alphaChannel } :
                                                        new PlanarImage[] { null },
                                                new ROI[] { in.roi }, request.isBlend() ?
                                                        MosaicDescriptor.MOSAIC_TYPE_BLEND :
                                                        MosaicDescriptor.MOSAIC_TYPE_OVERLAY,
                                                localHints);
                                roi = roi.add(new ROIGeometry(JTS.toGeometry(
                                        new ReferencedEnvelope(rasterBounds, null))));
            ...
            final RenderedImage mosaic = mergeBehavior
                    .process(sources, backgroundValues, sourceThreshold,
                            (hasAlpha || doInputTransparency) ? alphas : null, rois,
                            request.isBlend() ?
                                    MosaicDescriptor.MOSAIC_TYPE_BLEND :
                                    MosaicDescriptor.MOSAIC_TYPE_OVERLAY, localHints);

            Object property = mosaic.getProperty("ROI");
            ROI overallROI = (property instanceof ROI) ? (ROI) property : null;
            final RenderedImage postProcessed = footprintBehavior
    ...
    private class MosaicProducer implements GranuleCatalogVisitor {
                ...
                if (!requestedAdditionalDomains.isEmpty()) {
                    ...
                    for (Entry<String, List> entry : entries) {
                        if (entry.getValue().size() > 1) {
                            ...
                            filters.add(rasterManager.domainsManager
                    ...
                    if (multipleSelectionEntry == null) {
                        ...
                        final String domainName =
                                multipleSelectionEntry.getKey() + DomainDescriptor.DOMAIN_SUFFIX;
        ...
        public void visit(GranuleDescriptor granuleDescriptor, Object o) {
            ...
            if (!footprintBehavior.handleFootprints() || inclusionGeometry == null || (
                ...
                if (!found) {
                    throw new IllegalStateException(
                            "Unable to locate a filter for this granule:\n" + granuleDescriptor
                                    .toString());
                }

            } else {
                if (LOGGER.isLoggable(Level.FINE)) {
                    LOGGER.fine("We rejected for non ROI inclusion the granule " + granuleDescriptor
                            .toString());
                }
            }
        }
    ...
    private MosaicOutput prepareResponse() throws DataSourceException {
        ...
        try {
            ...
            if (visitor.granulesNumber == 0) {
                ...
                query.setFilter(FeatureUtilities.DEFAULT_FILTER_FACTORY
                        .bbox(FeatureUtilities.DEFAULT_FILTER_FACTORY.property(
    ...
    private void initTransformations() throws Exception {
        ...
        if (!request.isHeterogeneousGranules()) {
            ...
            if ((requestRes[0] < resX || requestRes[1] < resY)) {
                ...
                g2w.concatenate(AffineTransform
                        .getScaleInstance(selectedLevel.scaleFactor, selectedLevel.scaleFactor));
                g2w.concatenate(AffineTransform
                        .getScaleInstance(baseReadParameters.getSourceXSubsampling(),
    ...
    private void chooseOverview() throws IOException, TransformException {
        ...
        if (request.spatialRequestHelper.getComputedBBox() != null
                && request.spatialRequestHelper.getComputedRasterArea() != null && !request
                .isHeterogeneousGranules()) {
            imageChoice = ReadParamsController
                    .setReadParams(request.spatialRequestHelper.getComputedResolution(),
                            request.getOverviewPolicy(), request.getDecimationPolicy(),
                            baseReadParameters, request.rasterManager,
    ...
    private Query initQuery() throws Exception {
        final GeneralEnvelope levelRasterArea_ = CRS
                .transform(finalWorldToGridCorner, rasterManager.spatialDomainManager.coverageBBox);
        ...
        if (typeName != null) {
            ...
            bbox = FeatureUtilities.DEFAULT_FILTER_FACTORY
                    .bbox(FeatureUtilities.DEFAULT_FILTER_FACTORY.property(
                            rasterManager.getGranuleCatalog().getType(typeName)
                                    .getGeometryDescriptor().getName()), mosaicBBox);
    ...
    private void handleAdditionalFilters(Query query) {
        ...
        if (hasAdditionalDomains) {
            ...
            for (Entry<String, List> entry : additionalDomains.entrySet()) {
                ...
                additionalFilter.add(rasterManager.domainsManager
                        .createFilter(domainName, entry.getValue()));
    ...
    private MosaicOutput createBlankResponse() {
        ...
        final Number[] values = ImageUtilities
                .getBackgroundValues(rasterManager.defaultSM, backgroundValues);
        RenderedImage finalImage;
        if (ImageUtilities.isMediaLibAvailable()) {
            // create a constant image with a proper layout
            finalImage = ConstantDescriptor
                    .create(Float.valueOf(rasterBounds.width), Float.valueOf(rasterBounds.height),
                            values, renderingHints);
        ...
        boolean hasAlpha;
        if (inputTransparentColor != null && (footprintBehavior == null || !footprintBehavior
                .handleFootprints())) {
            if (LOGGER.isLoggable(Level.FINE)) {
    ...
    private GridCoverage2D prepareCoverage(MosaicOutput mosaicOutput) throws IOException {
        ...
        for (int i = 0; i < numBands; i++) {
            ...
            if (cm != null) {
                ...
                if (colorInterpretation == ColorInterpretation.UNDEFINED || bandNames
                        .contains(bandName)) {// make sure we create no duplicate band names
                    bandName = "Band" + (i + 1);
                }
            } else { // no color model
                bandName = "Band" + (i + 1);
        ...
        return coverageFactory.create(rasterManager.getCoverageIdentifier(), image,
                new GridGeometry2D(
                        ...
                        this.mosaicBBox.getCoordinateReferenceSystem(), hints), bands, null,
                properties);
    }
