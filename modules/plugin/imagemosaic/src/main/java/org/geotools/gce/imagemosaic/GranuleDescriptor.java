...
public class GranuleDescriptor {
        ...
        try {
            ...
            if (inStream == null) {
                ...
                throw new IllegalArgumentException(
                        "Unable to get an input stream for the provided file " + granuleUrl
                                .toString());
            }
            ...
            if (reader == null) {
                if (cachedReaderSPI == null) {
                    throw new IllegalArgumentException(
                            "Unable to get a ReaderSPI for the provided input: " + granuleUrl
                                    .toString());
                }
            ...
            this.granuleLevels.put(Integer.valueOf(0),
                    new GranuleOverviewLevelDescriptor(1, 1, originalDimension.width,
    ...
    private boolean customizeReaderInitialization(ImageReader reader, Hints hints) {
        ...
        if (hints != null && (hints.containsKey(Utils.AUXILIARY_FILES_PATH) || hints
    ...
    public GranuleDescriptor(final SimpleFeature feature, final ImageReaderSpi suggestedSPI,
        ...
        if (rasterFile == null) {
            throw new IllegalArgumentException(
                    Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, "granuleLocation",
                            granuleLocation));
        }
        if (LOGGER.isLoggable(Level.FINE))
    ...
    public GranuleLoadingResult loadRaster(final ImageReadParam imageReadParameters,
        ...
        if (useFootprint && inclusionGeometry != null && !JTS.toGeometry(cropBBox)
        ...
        try {
            ...
            if (isExternal) {
                ...
                assert ovrProvider.getExternalOverviewInputStreamSpi()
                        != null : "no cachedStreamSPI available for external overview!";
            ...
            if (pluginName != null && pluginName.equals(ImageUtilities.DIRECT_KAKADU_PLUGIN)) {
                ...
                if (newSubSamplingFactor != 0) {
                    ...
                    readParameters
                            .setSourceSubsampling(newSubSamplingFactor, newSubSamplingFactor, 0, 0);
                }
            }
            ...
            if (!XAffineTransform
                    .isIdentity(backToBaseLevelScaleTransform, Utils.AFFINE_IDENTITY_EPS))
                finalRaster2Model.concatenate(backToBaseLevelScaleTransform);
            if (!XAffineTransform
                    .isIdentity(afterDecimationTranslateTranform, Utils.AFFINE_IDENTITY_EPS))
            ...
            if (useFootprint) {
                ...
                try {
                    ...
                    transformed = roiProvider
                            .getTransformedROI(finalRaster2Model.createInverse(), imageIndex,
                                    imgBounds, readParameters, request.getReadType());
                    // Check for vectorial ROI
                    if (transformed instanceof ROIGeometry && ((ROIGeometry) transformed)
                            .getAsGeometry().isEmpty()) {
            ...
            Rectangle2D finalLayout = ImageUtilities
                    .layoutHelper(raster, (float) finalRaster2Model.getScaleX(),
            ...
            if (XAffineTransform.isIdentity(finalRaster2Model, Utils.AFFINE_IDENTITY_EPS)) {
                ...
                if (useFootprint && (roi instanceof ROIGeometry && ((ROIGeometry) roi)
                        .getAsGeometry().isEmpty()) || (roi instanceof ROI && ((ROI) roi)
                        .getBounds().isEmpty())) {
